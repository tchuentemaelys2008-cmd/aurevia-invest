...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (existing?.completed) return NextResponse.json({ error: "DÃ©jÃ  rÃ©clamÃ©" }, { status: 400 });
  ...
  if (progress < mission.target) return NextResponse.json({ error: "Mission non complÃ©tÃ©e", progress, target: mission.target }, { status: 400 });
  ...
  await prisma.$transaction([
    ...
    prisma.notification.create({ data: { userId: auth.userId, title: "Mission accomplie !", message: `Vous avez gagnÃ© ${mission.reward} FCFA â€” ${mission.title}`, type: "success" } }),
  ]);
