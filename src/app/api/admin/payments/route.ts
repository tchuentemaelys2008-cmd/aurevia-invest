    ...
    if (action === "approve") {
      ...
      if (userPass) {
        ...
        await prisma.transaction.create({
          data: { userId: payment.userId, type: "PASS_PURCHASE", amount: -payment.amount, description: "Achat Pass â€” virement bancaire confirmÃ©", status: "SUCCESS", reference },
        });
        await prisma.notification.create({
          data: { userId: payment.userId, title: "Pass activÃ© !", message: "Votre virement a Ã©tÃ© reÃ§u. Votre pass est maintenant actif.", type: "success" },
        });
        // Referral commission
        ...
        if (buyer?.referredById) {
          ...
          await prisma.transaction.create({ data: { userId: buyer.referredById, type: "REFERRAL_BONUS", amount: commission, description: "Commission parrainage 10% â€” virement bancaire", status: "SUCCESS" } });
        }
      }
      ...
      await prisma.notification.create({
        data: { userId: payment.userId, title: "Virement non confirmÃ©", message: "Votre preuve de paiement n'a pas pu Ãªtre validÃ©e. Contactez le support.", type: "error" },
      });
    }
