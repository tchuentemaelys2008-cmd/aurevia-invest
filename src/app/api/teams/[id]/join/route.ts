...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (existing) return NextResponse.json({ error: "DÃ©jÃ  dans une Ã©quipe" }, { status: 400 });

  const team = await prisma.team.findUnique({ where: { id: params.id } });
  if (!team || !team.isActive) return NextResponse.json({ error: "Ã‰quipe introuvable" }, { status: 404 });
