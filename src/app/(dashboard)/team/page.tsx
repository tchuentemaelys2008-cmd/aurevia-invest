...
export default function TeamPage() {
  ...
  const levelBadge = (level: number) => {
    if (level >= 5) return { label: "IA Advisor", icon: <Bot size={11} />, color: "text-[#6c5ce7] bg-[#6c5ce7]/10 border-[#6c5ce7]/20" };
  ...
  return (
    <div className="p-4 lg:p-8 max-w-2xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold text-[#5b6ef5] uppercase tracking-widest mb-1">{t("team_kicker")}</p>
        ...
        {!myMemberTeamId && !ledTeamId && (
          <button type="button" onClick={() => setShowCreate(v => !v)}
            className="flex items-center gap-2 bg-gradient-to-r from-[#5b6ef5] to-[#6c5ce7] text-white text-sm font-semibold px-4 py-2.5 rounded-xl hover:opacity-90 transition-all touch-manipulation">
      ...
      {!hasActivePass && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <a href="/passes" className="ml-auto flex-shrink-0 flex items-center gap-1 text-xs font-semibold text-[#5b6ef5] bg-[#5b6ef5]/10 px-3 py-1.5 rounded-xl">
          ...
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="glass-card rounded-2xl p-5 mb-6">
            ...
            <div className="flex gap-3">
              <input value={newName} onChange={e => setNewName(e.target.value)} placeholder={t("team_name_placeholder")}
                className="flex-1 bg-white/5 border border-[var(--control-border)] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#5b6ef5]/50"
                style={{ color: "var(--control-text)" }} />
              <button type="button" onClick={create} disabled={creating || !newName.trim()}
                className="bg-[#5b6ef5] text-white text-sm font-semibold px-4 py-2.5 rounded-xl hover:opacity-90 disabled:opacity-50 transition-all touch-manipulation">
      ...
      {pendingRequests.length > 0 && (
        <div className="mb-6">
          <p className="text-xs font-semibold text-[#5b6ef5] uppercase tracking-widest mb-3">{t("team_requests_title")} ({pendingRequests.length})</p>
          ...
          {teams.map((team, i) => {
            ...
            return (
              <motion.div key={team.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                className={`glass-card rounded-2xl p-5 flex items-center gap-4 ${isMyMemberTeam ? "ring-1 ring-[#5b6ef5]/30" : ""}`}>
                <div className="w-10 h-10 bg-gradient-to-br from-[#5b6ef5]/20 to-[#6c5ce7]/20 rounded-xl flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isMyLedTeam && (
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-[#5b6ef5]/15 text-[#5b6ef5] border border-[#5b6ef5]/20 flex items-center gap-0.5">
                ...
                {canRequest && (
                  <button type="button" onClick={() => setRequestTeam(team)}
                    className="flex items-center gap-1.5 text-xs font-semibold text-[#5b6ef5] bg-[#5b6ef5]/10 hover:bg-[#5b6ef5]/20 border border-[#5b6ef5]/20 px-3 py-2 rounded-xl transition-all touch-manipulation">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <textarea value={reqMessage} onChange={e => setReqMessage(e.target.value)} rows={3}
                    ...
                    className="w-full rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#5b6ef5]/50 resize-none"
                ...
                <button type="button" onClick={sendRequest} disabled={sendingReq || !reqMessage.trim() || !reqPhone.trim()}
                  className="w-full bg-gradient-to-r from-[#5b6ef5] to-[#6c5ce7] text-white text-sm font-semibold py-3 rounded-xl hover:opacity-90 disabled:opacity-50 transition-all touch-manipulation">
