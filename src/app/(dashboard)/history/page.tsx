    ...
    <div className="max-w-xl mx-auto px-4 pt-8 pb-10">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-5">
        ...
        <h1 className="text-2xl font-display font-bold text-white flex items-center gap-2">
          <HistoryIcon size={22} className="text-[#5b6ef5]" /> {fr ? "Historique" : "History"}
        </h1>
      </motion.div>

      {/* Tabs */}
      <div className="flex gap-2 mb-4">
        <button onClick={() => setTab("all")} className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all ${tab === "all" ? "bg-[#5b6ef5] text-white" : "bg-white/5 text-white/50"}`}>
          {fr ? "Tout" : "All"}
        </button>
