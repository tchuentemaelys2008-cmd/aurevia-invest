    ...
    <div className="max-w-xl mx-auto px-4 pt-8 pb-10 space-y-5">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h1 className="text-2xl font-display font-bold text-white flex items-center gap-2">
          <Wallet size={22} className="text-[#5b6ef5]" /> {fr ? "Dépôt" : "Deposit"}
        </h1>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
        <Card className="space-y-3">
          ...
          <input
            ...
            placeholder="5000"
            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-2xl font-display font-bold text-white placeholder:text-white/25 focus:outline-none focus:border-[#5b6ef5]/50"
          />
          <div className="flex flex-wrap gap-2">
            {QUICK.map((q) => (
              <button key={q} onClick={() => setAmount(String(q))}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${amt === q ? "bg-[#5b6ef5] text-white border-[#5b6ef5]" : "bg-white/5 text-white/60 border-white/10 hover:border-[#5b6ef5]/40"}`}>
                +{formatCurrency(q)}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-2">
          {REGIONS.map((r) => (
            <button key={r.code} onClick={() => setRegion(r)}
              className={`w-full flex items-center gap-3 p-3.5 rounded-xl border transition-all text-left ${region?.code === r.code ? "border-[#5b6ef5]/50 bg-[#5b6ef5]/10" : "border-white/8 bg-white/4 hover:bg-white/7"}`}>
              ...
              <div className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${region?.code === r.code ? "border-[#5b6ef5]" : "border-white/20"}`}>
                {region?.code === r.code && <div className="w-2 h-2 bg-[#5b6ef5] rounded-full" />}
              </div>
            </button>
          ))}
        </div>
