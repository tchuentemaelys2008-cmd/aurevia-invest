  ...
  return (
    <div className="p-6 max-w-xl">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-9 h-9 bg-[#5b6ef5]/20 rounded-xl flex items-center justify-center"><Bell size={18} className="text-[#5b6ef5]" /></div>
      ...
      <div className="bg-[#0c1428] border border-white/10 rounded-2xl p-5 space-y-5">
        ...
        <div className="flex rounded-xl overflow-hidden border border-white/10">
          <button onClick={() => setMode("global")} className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold transition-all ${mode === "global" ? "bg-[#5b6ef5] text-white" : "text-white/40 hover:text-white"}`}>
            <Globe size={15} /> Global
          </button>
          <button onClick={() => setMode("targeted")} className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold transition-all ${mode === "targeted" ? "bg-[#5b6ef5] text-white" : "text-white/40 hover:text-white"}`}>
            <User size={15} /> Ciblé
          </button>
        </div>
        ...
        {mode === "targeted" && (
          <div>
            ...
            <input value={form.targetUserId} onChange={e => set("targetUserId", e.target.value)} placeholder="cuid de l'utilisateur"
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-[#5b6ef5]/50" />
          </div>
        )}
        ...
        <div>
          ...
          <textarea value={form.message} onChange={e => set("message", e.target.value)} rows={3} placeholder="Contenu du message"
            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-[#5b6ef5]/50 resize-none" />
        </div>
        <div>
        ...
        <button onClick={send} disabled={sending} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[#5b6ef5] to-[#6c5ce7] text-white font-semibold py-3 rounded-xl hover:opacity-90 disabled:opacity-50 transition-all">
