...
export default function AdminEventsPage() {
  ...
  const typeColors: Record<string, string> = { double_earnings: "#10b981", discount: "#f59e0b", bonus: "#b51d2c", xp_boost: "#e23744" };

  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-[#b51d2c]/20 rounded-xl flex items-center justify-center"><Zap size={18} className="text-[#b51d2c]" /></div>
        ...
        <button onClick={() => setShowForm(v => !v)} className="flex items-center gap-2 bg-[#b51d2c] text-white text-sm font-semibold px-4 py-2 rounded-xl hover:opacity-90 transition-all">
          <Plus size={15} /> Créer
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-[#0c1428] border border-white/10 rounded-2xl p-5 mb-6 grid grid-cols-2 gap-4">
          {[["Titre (FR)", "title", "text"], ["Titre (EN)", "titleEn", "text"], ["Description", "description", "text"], ["Valeur (%)", "value", "number"], ["Début", "startsAt", "datetime-local"], ["Fin", "endsAt", "datetime-local"]].map(([label, key, type]) => (
            <div key={key as string} className={key === "description" ? "col-span-2" : ""}>
              ...
              <input type={type as string} value={(form as Record<string,unknown>)[key as string] as string} onChange={e => set(key as string, type === "number" ? Number(e.target.value) : e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-[#b51d2c]/50" />
            </div>
          ))}
          <div>
          ...
          <div className="col-span-2 flex gap-3">
            <button onClick={save} className="flex-1 bg-[#b51d2c] text-white text-sm font-semibold py-2.5 rounded-xl hover:opacity-90">Créer l'événement</button>
